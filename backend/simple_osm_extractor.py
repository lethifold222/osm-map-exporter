#!/usr/bin/env python3
"""
Simple OSM data extractor with better error handling
"""
...
class SimpleOSMExtractor:
    ...
    async def extract_data(self, bbox: Dict[str, float], layers: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"🔍 Querying Overpass API for bbox: {overpass_bbox}")
    ...
    def _process_osm_data(self, data: Dict, bbox: Dict[str, float], layers: List[str]) -> Dict[str, Any]:
        ...
        for element in elements:
            try:
                ...
                if element.get("type") == "way":
                    if "geometry" in element and element["geometry"] and isinstance(element["geometry"], dict):
                ...
                elif element.get("type") == "node" and ("amenities" in layers or "pois" in layers):
                    # Node POI
                    if "lat" in element and "lon" in element and isinstance(element.get("lat"), (int, float)) and isinstance(element.get("lon"), (int, float)):
                ...
                print(f"⚠️ Error processing element {element.get('id', 'unknown')}: {e}")
                continue
        ...
        print(f"✅ Processed: {len(roads)} roads, {len(buildings)} buildings, {len(all_amenities)} amenities")
